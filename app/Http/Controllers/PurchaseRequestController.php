...
{
    $requestCount = PurchaseRequest::with(['detailrequest.item.unit', 'detailrequest.item.category', 'cbd', 'user'])
        ->whereHas('detailrequest', function ($query) {
            $query->where(function ($subQuery) {
                // Cek jika status kosong atau null
        ...
        ->count(); // Menghitung jumlah PurchaseRequest yang sesuai
    ...
    public function GetpurchaserequestCountPendingccc()
    {
    ...
    {
        $requestCount = PurchaseRequestDetail::where('total', '>', 0)->count();
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('item_code', function ($row) {
                    // Menampilkan item_code dalam <ul><li></li></ul> dengan potongan 25 karakter
                    $details = '<ul>';
                    foreach ($row->detailrequest as $detail) {
                        // Jika item_code null, gunakan '-' sebagai default
                        $itemCode = $detail->item->item_code ?? '-';

                        // Potong nama item menjadi 25 karakter jika panjangnya lebih dari 25
                        if (strlen($itemCode) > 25) {
                            $itemCode = substr($itemCode, 0, 25) . '...';
                        }

                        $details .= '<li>' . $itemCode . '</li>';
                    }
                    $details .= '</ul>';
                ...
                ->addColumn('status', function ($row) {
                    $details = '<ul>';
                    foreach ($row->detailrequest as $detail) {
                        // Cek apakah total sama dengan 0
                        if ($detail->total == 0) {
                            $details .= '<li>' . '<span class="badge bg-primary">' . 'use stok' . '</span>' . '</li>';
                        } elseif ($detail->status == 'po') {
    ...
    public function Getpurchaserequestwaiting(Request $request)
    {
        if ($request->ajax()) {
            ...
            $query->whereHas('detailrequest', function ($query) {
                $query->where(function ($subQuery) {
                    $subQuery->whereNull('status') // status NULL
                        ->orWhere('status', '');    // status kosong
                })
                    ->where('total', '>', 0)
    ...
    private function generatePurchaseRequestNo()
    {
        ...
        // Temukan purchase request terakhir pada tahun dan bulan yang sama
        $latestRequest = PurchaseRequest::whereYear('created_at', $currentYear)
            ->whereMonth('created_at', $currentMonth)
            ->orderBy('id', 'desc')
            ->first();

        $newNumber = 1; // Default nomor urut mulai dari 1

        if ($latestRequest) {
            // Ambil tahun dari purchase_request_no terakhir
            $purchaseRequestNo = $latestRequest->purchase_request_no;

            // Pisahkan berdasarkan separator "/" dan ambil bagian pertama (nomor urut) dan terakhir (tahun)
            $parts = explode('/', $purchaseRequestNo);
            $lastYear = end($parts); // Ambil bagian terakhir (tahun)



            if ($lastYear == $currentYear) {
                // Jika tahun sama, lanjutkan nomor urut terakhir
                // Ambil nomor urut 6 digit pertama dari purchase_request_no
    ...
    public function Storepurchaserequest(Request $request)
    {
        ...
        if ($purchaseRequest) {
            foreach ($request->details as $detail) {
                ...
                $stock = $detail['stock'] ?? 0; // Ambil stock atau default ke 0
                $total = ($detail['total'] ?? 0) - $stock; // Hitung total dengan mengurangi stock
    ...
    public function Updatepurchaserequest(Request $request, $id)
    {
        ...
        foreach ($request->details as $detail) {
            ...
            $calculatedTotal = ($detail['total'] ?? 0) - ($detail['stock'] ?? 0); // Hitung total baru
            ...
            PurchaseRequestDetail::create([
                ...
                'consumtion' => $detail['consumption'] ?? null,
                'allowance' => $detail['allowance'] ?? null,
                'stock' => $detail['stock'] ?? 0, // Simpan nilai stock
                'total' => $calculatedTotal, // Simpan hasil total yang dihitung
                'remark' => $detail['remark'] ?? null,
    ...
    public function Getpurchaserequestitems(Request $request)
    {
        ...
        $items =  PurchaseRequestDetail::where('purchase_request_id', $purchaseRequestId)
           ...
           ->where('total', '>', 0)  // total lebih besar dari 0
