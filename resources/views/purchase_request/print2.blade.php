...
<body>
    ...
    <div class="content">
        ...
        foreach ($purchaseRequest->detailrequest as $detail) {
            $itemCode = $detail->item->item_code ?? 'Not Available';
            ...
            $itemGroups[$itemCode]['total_qty'] += $detail->total; // Hitung total_qty per item_code
        }
        ?>

        <table class="table detail-request table-detail-request"
            style="font-size: {{ count($purchaseRequest->detailrequest) > 3 ? '6px' : '10px' }}">
            <thead>
                <tr>
                    <th>NO</th>
                    <th>SUPPLIER</th>
            ...
            <tbody>
                ...
                @foreach ($itemGroups as $itemCode => $group)
                    @foreach ($group['details'] as $detail)
                        <tr>
                            <td>{{ $rowNumber++ }}</td>
                            <td>{{ $detail->supplier->supplier_name ?? 'Not Available' }}</td>
                            <td>{{ $itemCode }}</td>
