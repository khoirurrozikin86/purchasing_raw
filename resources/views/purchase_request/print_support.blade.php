...
<body>
    ...
    <div class="content">
        ...
        <table class="table table-no-border table-request" style="width: auto; margin-left: 0;">
            ...
            <tr>
                <td>MO / STYLE</td>
                <td>: {{ $purchaseRequest->mo ?? '-' }} /
                    {{ $purchaseRequest->style ?? '-' }}</td>
            ...
            <tbody>
                @php
                    ...
                    foreach ($purchaseRequest->detailrequest as $detail) {
                        ...
                        $groupedData[$itemCode]['details'][] = $detail;
                        $groupedData[$itemCode]['total_qty'] += $detail->total; // Hitung TOTAL QTY
                    }
                @endphp
                @php $rowNumber = 1; @endphp
                @foreach ($groupedData as $itemCode => $group)
                    @foreach ($group['details'] as $detail)
                        <tr>
                            ...
                            <td>{{ $detail->total }}</td> <!-- TOTAL QTY -->
                            <td>{{ $detail->remark }}</td>
                        </tr>
                    @endforeach
                    <!-- Baris Total untuk setiap item_code -->
                    <tr>
                        <td colspan="11" style="font-weight: bold; text-align: center;">TOTAL</td>

                        <td colspan="2" style="font-weight: bold; text-align: left;">{{ $group['total_qty'] }}
                            {{ $group['unit'] }}</td>
