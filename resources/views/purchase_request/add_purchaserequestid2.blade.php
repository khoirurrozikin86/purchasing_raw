...
@section('admin')
        ...
        <thead>
            <tr>
                ...
                @php
                    ...
                    $colors = collect($colors)->sortBy(function ($color) use ($colorPriority) {
                        return array_search($color, $colorPriority) !== false ? array_search($color, $colorPriority) : PHP_INT_MAX;
                    })->toArray();

                    // Sort sizes based on priority array
                    $sizePriority = ['XS','S', 'M', 'L', 'XL', 'XXL', '3XL', '4XL', '5XL'];
                    $sizes = collect($sizes)->sortBy(function ($size) use ($sizePriority) {
                        return array_search($size, $sizePriority) !== false ? array_search($size, $sizePriority) : PHP_INT_MAX;
                    })->toArray();
                @endphp
                    ...
                    @foreach ($colors as $color)
                        <td>
                            ...
                            @php
                                ...
                                $colorTotals[$color] = ($colorTotals[$color] ?? 0) + ($qtyData[$size][$color] ?? 0);
    ...
    <script>
        ...
        // Fungsi untuk menghitung ulang total ketika qty atau allowance diubah
        function calculateTotal(input) {
            ...
            const total = Math.round((qty * consumption) + allowance); // Hasil total tanpa desimal
            row.find('.total').val(total); // Tampilkan total tanpa .00
        }
