...
<body>
    ...
    <div class="content">
        ...
        <table class="table table-no-border table-request" style="width: auto; margin-left: 0;">
            ...
            <tr>
                <td>MO / STYLE</td>
                <td>: {{ $purchaseRequest->cbd->payment_terms ?? '-' }} /
                    {{ $purchaseRequest->cbd->sample_code ?? '-' }}</td>
            ...
            <table class="table table-po" style="width:25%">
                <thead>
                    <tr>
                        ...
                        @foreach ($colors as $color)
                            <th>{{ $color }}</th>
                        @endforeach
                        <th>Total</th> <!-- Add column for total quantity per size -->
                    </tr>
                </thead>
                <tbody>
                    @php
                            ...
                            @foreach ($colors as $color)
                                <td>
                                    ...
                                    @php
                                        ...
                                        $colorTotals[$color] =
                                            ($colorTotals[$color] ?? 0) + ($qtyData[$size][$color] ?? 0);
                                        // Accumulate the totals for each size
                                        $sizeTotals[$size] = ($sizeTotals[$size] ?? 0) + ($qtyData[$size][$color] ?? 0);
                ...
                <tfoot>
                    <tr>
                        <th>Total</th>
                        @foreach ($colors as $color)
                            <th>{{ $colorTotals[$color] ?? 0 }}</th> <!-- Total for each color -->
                        @endforeach
       ...
       <?php
        ...
        foreach ($purchaseRequest->detailrequest as $detail) {
            $itemCode = $detail->item->item_code ?? ''; // Grupkan berdasarkan item_code
        
            // Pastikan grup berdasarkan item_code sudah ada
            if (!isset($itemGroups[$itemCode])) {
                $itemGroups[$itemCode] = [
                    'details' => [], // Inisialisasi array untuk menyimpan detail
                    'total_qty' => 0, // Total quantity untuk item_code
                    'unit' => $detail->item->unit->unit_code ?? '-', // Tambahkan unit
                ];
            }
        
            // Tambahkan detail ke grup berdasarkan item_code
            $itemGroups[$itemCode]['details'][] = $detail;
        
            // Tambahkan total quantity untuk item_code
            $itemGroups[$itemCode]['total_qty'] += $detail->total;
        }
        ?>

        <table class="table detail-request table-detail-request"
            style="font-size: {{ count($purchaseRequest->detailrequest) > 10 ? '6px' : '10px' }}">
            <thead>
                <tr>
                    <th>NO</th>
                    <th>SUPPLIER</th>
                    ...
                    <th>ALLOWANCE</th>
                    <th>USE STOCK</th>
                    <th>TOTAL QTY</th> <!-- Kolom ini akan digunakan untuk total -->
                    <th>REMARKS</th>
                </tr>
            </thead>
            <tbody>
                @php $rowNumber = 1; @endphp
                @foreach ($itemGroups as $itemCode => $group)
                    @php
                        // Group colors for each item_code
                        $colorGroups = [];
                        foreach ($group['details'] as $detail) {
                            ...
                            $colorGroups[$color]['details'][] = $detail;
                            $colorGroups[$color]['total_qty'] += $detail->total;
                        }
                    @endphp

                    @foreach ($colorGroups as $color => $colorGroup)
                        @foreach ($colorGroup['details'] as $detail)
                            <tr>
                                <td>{{ $rowNumber++ }}</td>
                                ...
                                <td>{{ $detail->size }}</td>
                                <td>{{ $color }}</td> <!-- Tampilkan color -->
                                <td>{{ $detail->qty }}</td>
                                <td>{{ $group['unit'] }}</td>
                                <td>{{ $detail->consumtion }}</td>
                                <td>{{ $detail->allowance }}</td>
                                <td>{{ $detail->stock }}</td>
                                <td>{{ $detail->total }}</td> <!-- Tampilkan total_qty dari setiap baris -->
                                <td>{{ $detail->remark }}</td>
                            </tr>
                        @endforeach
                    @endforeach

                    <!-- Baris Total Per Item Code -->
                    <tr>
                        <td colspan="12" style="text-align: center; font-weight: bold;">TOTAL</td>
                        <td colspan="2" style="font-weight: bold;">
                            {{ $group['total_qty'] }} {{ $group['unit'] }}
                        </td> <!-- Total Qty per item_code dengan unit -->
                    </tr>
                @endforeach
            </tbody>
        </table>
